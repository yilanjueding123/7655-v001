...
void gc1004_sensor_calibration(void)
{
	...
	gp_Cdsp_SetAWBYUV_test((INT16S *)g_GC1004_awb_thr);
}
            	  ...
            	  for (i=0; i<sizeof(GC1064_MIPI_720P)/2; i++) 
			         {
				       reg_tmp = GC1064_MIPI_720P[i][0];
						...
						if(ap_state_config_video_resolution_get()==2)//720p
							{
							   if(reg_tmp ==0x08)
							    GC1064_MIPI_720P[i][1]  =0x08;
							}
						else if(ap_state_config_video_resolution_get() == 0)
							{
							   if(reg_tmp ==0x08)
							    GC1064_MIPI_720P[i][1]  =0xff;
							}
