...
void state_browse_entry(void *para, INT16U play_index)
{
	...
	//if(uninit_flag == 0)
	msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_BROWSE_INIT, NULL, sizeof(INT8U), MSG_PRI_NORMAL);	
	//else
	...
	while (exit_flag == EXIT_RESUME) {
		...
		switch (msg_id) {
        	...
        	case MSG_APQ_FUNCTION_KEY_ACTIVE:
				//DBG_PRINT("----browse_play\r\n");
				if(g_browser_reply_action_flag) {
	        	    if (func_key_act == 0) {
						DBG_PRINT("----browse_play1\r\n");
	        	        ap_browse_func_key_active();
	        		}
	        	}
        		break;
